...
const TimeLockedWalletFactory = artifacts.require(
...
contract("TimeLockedWalletFactory", (accounts) => {
  before(async () => {
    creator = accounts[0];
    owner = accounts[1];
    timeLockedWalletFactory = await TimeLockedWalletFactory.new({
      from: creator,
    });
  });
  ...
  it("Factory created contract is working well", async () => {
    ...
    await timeLockedWalletFactory.newTimeLockedWallet(owner, now, {
      from: creator,
      value: ethToSend,
    });

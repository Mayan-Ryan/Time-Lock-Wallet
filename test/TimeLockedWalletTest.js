...
const ToptalToken = artifacts.require("./Token.sol");
...
contract("TimeLockedWallet", (accounts) => {
  ...
  it("Nobody can withdraw the funds before the unlock date", async () => {
    ...
    //create the contract
    let timeLockedWallet = await TimeLockedWallet.new(
      creator,
      owner,
      futureTime
    );
  ...
  it("Owner can withdraw the ToptalToken after the unlock date", async () => {
    ...
    await Token.transfer(timeLockedWallet.address, amountOfTokens, {
      from: creator,
    });
    //check that timeLockedWallet has ToptalTokens
    assert(
      amountOfTokens == (await Token.balanceOf(timeLockedWallet.address))
    );
    //now withdraw tokens
    await timeLockedWallet.withdrawTokens(Token.address, { from: owner });
